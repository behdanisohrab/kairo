...
func TestMigrateConfigAddsDefaults(t *testing.T) {
...
func TestMigrateConfigNeverAddsIdentityKeys(t *testing.T) {
	path := writeConfig(t, `{"vps_ip": "1.2.3.4", "api_key": "secret", "upstream_dns": ["1.1.1.1"]}`)
	if err := migrateConfig(path); err != nil {
		t.Fatalf("migrateConfig: %v", err)
	}
	m := readConfigMap(t, path)
...
func TestMigrateConfigPreservesUnknownKeys(t *testing.T) {
	path := writeConfig(t, `{
  "vps_ip": "1.2.3.4",
  "api_key": "secret",
  "upstream_dns": ["1.1.1.1"],
  "restricted": ["youtube.com"]
}`)
	if err := migrateConfig(path); err != nil {
		t.Fatalf("migrateConfig: %v", err)
	}
	m := readConfigMap(t, path)
	restricted, ok := m["restricted"]
	if !ok {
		t.Fatalf("unknown key restricted lost after migrate")
	}
	list, ok := restricted.([]interface{})
	if !ok || len(list) != 1 || list[0] != "youtube.com" {
		t.Errorf("restricted = %v, want [youtube.com]", restricted)
	}
}

func TestMigrateConfigAlreadyUpToDate(t *testing.T) {
	path := writeConfig(t, `{
  "vps_ip": "1.2.3.4",
  "api_key": "secret",
  "upstream_dns": ["1.1.1.1"]
...
func TestMigrateConfigRejectsInvalid(t *testing.T) {
	// Empty api_key must not survive migration; the file must be left alone.
	path := writeConfig(t, `{"vps_ip": "1.2.3.4", "api_key": "", "upstream_dns": ["1.1.1.1"]}`)
	if err := migrateConfig(path); err == nil {
		t.Fatalf("migrateConfig accepted an invalid config")
	}
