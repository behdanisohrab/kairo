...
from scratch. It writes a valid `config.yaml`, a fresh random `api_key`, and the
three policy files into the directory you give it.
...
it never overwrites an existing `config.yaml`, so running it again is safe if
you want to see what a default looks like.

## Edit the generated files

The generated `config.yaml` has placeholders. Open it and set the values that
matter:

- `host` must be your domain name, for example `dns.example.com`.
- `vps_ip` must be the public IPv4 address of this server.
- `api_key` is already filled with a freshly generated random secret; leave it
  as is, or replace it if you prefer your own.
...
exactly where the image looks for `config.yaml`. The generated files need no
copying or moving.
