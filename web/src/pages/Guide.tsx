    ...
    <div className="mx-auto max-w-[760px] space-y-4 sm:space-y-6">
      ...
      <div className="grid gap-3 sm:gap-4">
        ...
        <div className="card p-3 sm:p-5">
          ...
          <div className="mt-3"><Code title="API">{`curl -H "Authorization: Bearer ${key}" ${origin}/api/me/devices`}</Code></div>
          <div className="mt-3 grid gap-1.5 text-xs text-[var(--color-ink-2)]"><div className="flex gap-2"><code className="inline">device_type</code> <span className="text-[var(--color-ink-3)]">Desktop Android iOS Tablet Bot</span></div><div className="flex gap-2"><code className="inline">ja3_hash</code> <span className="text-[var(--color-ink-3)]">TLS fingerprint</span></div></div>
        </div>
        {user?.role === 'admin' && (
          <div className="card p-0 overflow-hidden">
            ...
            <div className="table-wrap"><table className="table"><thead><tr><th>Method</th><th>Endpoint</th><th>Description</th></tr></thead><tbody>{[ ['GET','/api/users','List all users'], ['POST','/api/users','Create user'], ['DELETE','/api/users/:id','Delete user'], ['GET','/api/users/:id/devices','User devices'], ['POST','/api/users/:id/api-key/regenerate','Regen key'], ['GET','/api/devices','All devices'], ['POST','/api/me/api-key/regenerate','Regen own'], ['GET','/api/allow','List allowlist'], ['POST','/api/allow?ip=','Add IP'], ['GET','/api/restricted','List domains'], ['POST','/api/restricted?domain=','Add domain'], ].map(([m,ep,desc]) => <tr key={ep}><td><span className="mono text-[11px] font-bold" style={{ color: m==='GET'?'var(--color-emerald)':m==='POST'?'var(--color-brand)':'var(--color-rose)' }}>{m}</span></td><td className="mono text-xs">{ep}</td><td className="text-xs text-[var(--color-ink-3)]">{desc}</td></tr>)}</tbody></table></div>
          </div>
        )}
      </div>
    </div>
