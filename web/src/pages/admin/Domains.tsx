...
import { FiGlobe, FiPlus, FiTrash2, FiCheck, FiX, FiSearch, FiSend, FiUpload, FiRefreshCw } from 'react-icons/fi'
...
export default function Domains() {
  ...
  const handleAddOne = async (e: React.FormEvent) => {
    ...
    if (!d) return
    setAdding(true)
    const r = await api.addRestricted(d)
    if (r.ok) {
      success(`Added ${d}`)
      setAddOne('')
      load()
  ...
  const handleDelete = async (d: string) => {
    ...
    const r = await api.removeRestricted(d)
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--color-ink-3)]">{t('domains.subtitle', { filtered: filtered.length, total: domains.length })}</p>
        </div>
